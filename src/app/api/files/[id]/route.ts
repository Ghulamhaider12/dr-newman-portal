...
import { mapHelpingMaterials } from '@/lib/helpingMaterials';
...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  ...
  if (body.isYoutube === true) {
    ...
    if (!watch) return NextResponse.json({ error: 'Invalid YouTube URL.' }, { status: 400 });
    data.isYoutube = true;
    data.url = watch;
    data.fileType = 'MP4';
    data.storageKey = '';
  ...
  // Replace the helping-material set only when the field is provided. Materials
  // are just upload pointers, so a clear-and-recreate keeps the code simple and
  // correct (deleteMany is scoped to this file's rows).
  if ('helpingMaterials' in body) {
    data.helpingMaterials = {
      deleteMany: {},
      create: mapHelpingMaterials(body.helpingMaterials),
    };
  }
